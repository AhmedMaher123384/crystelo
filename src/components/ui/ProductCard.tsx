...
const ProductCard: React.FC<ProductCardProps> = ({ product, viewMode = 'grid' }) => {
  ...
  if (viewMode === 'list') {
    return (
      <Link
        ...
        className={`block bg-white rounded-xl border border-[#d9a890]/20 shadow-[0_8px_24px_-4px_rgba(217,168,144,0.12),0_4px_8px_-2px_rgba(0,0,0,0.08)] w-full min-h-[160px] overflow-hidden ${
      ...
      >
        <div className="flex p-4 gap-4 h-full">
          <div className="flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden relative border border-[#d9a890]/10">
            <img
            ...
            {!product.isAvailable && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                <span className="text-white font-medium bg-[#d9a890] px-3 py-1 rounded-full text-xs tracking-wide">
                  {t('common.outOfStock')}
                </span>
              </div>
            )}
            {hasDiscount && (
              <div className="absolute -top-2 -right-2 z-10">
                <div className="bg-[#d9a890] text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-lg">
                  -{discountPercent}%
                </div>
              </div>
            )}
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-between">
              ...
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all duration-300 ${
                    isInWishlist 
                      ? 'text-[#d9a890] bg-[#fdf9f7] scale-110' 
                ...
                >
                  <Heart 
                    className={`w-5 h-5 ${isInWishlist ? 'fill-[#d9a890] animate-pulse' : ''}`} 
                  />
                </button>
              </div>
            ...
            <div className="mt-2 space-y-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <CheckCircle className={`w-3.5 h-3.5 ${product.isAvailable ? 'text-green-600' : 'text-red-600'}`} />
 ...
 <span className="w-8 text-center font-medium text-[#c18c78]">
  ...
  return (
    <Link
      to={`/product/${createProductSlug(product.id, getLocalizedContent('name'))}`}
      className={`block bg-white rounded-xl border border-[#d9a890]/20 shadow-[0_8px_24px_-4px_rgba(217,168,144,0.12),0_4px_8px_-2px_rgba(0,0,0,0.08)] w-full max-w-xs min-h-[480px] flex flex-col mx-auto ${
        isRTL ? 'text-right' : 'text-left'
      } hover:shadow-[0_12px_32px_-4px_rgba(217,168,144,0.18),0_6px_12px_-2px_rgba(0,0,0,0.12)] transition-all duration-400`}
      onClick={handleProductClick}
      aria-label={t('product.view_product_details', { name: getLocalizedContent('name') })}
    >
      <div className="relative flex-shrink-0">
        <div className="aspect-square w-full overflow-hidden rounded-xl border-b border-[#d9a890]/10">
          <img
            src={buildImageUrl(product.mainImage)}
            alt={getLocalizedContent('name')}
            className="w-full h-full object-cover"
          ...
          {!product.isAvailable && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
              <span className="text-white font-medium bg-[#d9a890] px-4 py-2 rounded-full text-sm tracking-wide">
                {t('common.outOfStock')}
              </span>
            </div>
          )}
          {hasDiscount && (
            <div className="absolute -top-3 -right-3 z-10">
              <div className="bg-[#d9a890] text-white text-sm font-bold px-3 py-1.5 rounded-full shadow-lg">
                -{discountPercent}%
              </div>
            </div>
          )}
        ...
        <button
          ...
          className={`absolute top-3 right-3 p-2 rounded-full transition-all duration-300 z-20 ${
            isInWishlist 
              ? 'text-[#d9a890] bg-white scale-110 shadow-sm' 
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="space-y-2 mb-4">
          ...
          <div className="flex items-center gap-2">
            <CheckCircle className={`w-4 h-4 ${product.isAvailable ? 'text-green-600' : 'text-red-600'}`} />
