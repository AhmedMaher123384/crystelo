...
const FAQCard: React.FC<{ 
 ...
 <div className="bg-[#d9a890] text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold flex-shrink-0 mt-1">
            {index + 1}
          </div>
        ...
        <div className="flex-shrink-0 mr-2">
          {isOpen ? (
 <ChevronUp className="w-5 h-5 text-[#c18c78]" />
          ) : (
 ...
 <div className="border-r-4 border-[#c18c78] pr-4">
...
const ProductDetail: React.FC = () => {
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={`w-5 h-5 ${
 star <= rating ? 'text-[#c18c78] fill-[#c18c78]' : 'text-gray-300'
            }`}
          />
        ))}
      </div>
 ...
 <div className="inline-block w-8 h-8 border-4 border-[#c18c78] border-t-transparent rounded-full animate-spin"></div>
 ...
 <div className="inline-block w-12 h-12 rounded-full bg-[#d9a890]/10 flex items-center justify-center mb-4">
 <AlertCircle className="w-6 h-6 text-[#c18c78]" />
          </div>
          <h2 className="text-xl font-bold text-[#0A2A55] mb-2">{t('error')}</h2>
          <p className="text-gray-700 mb-6">{error || t('product_not_found')}</p>
          <Link
            to="/"
 className="inline-block px-6 py-3 bg-[#d9a890] text-white font-medium rounded-md hover:bg-[#c18c78]"
          >
            {t('back_to_home')}
          </Link>
        </div>
    ...
    <section className="min-h-screen bg-white py-6 sm:py-8" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-600 mb-6" dir="ltr">
          <button 
            onClick={() => navigate('/')} 
 className="hover:text-[#c18c78] transition-colors"
          >
            {t('home')}
          </button>
 ...
 <div className="h-1 w-24 bg-[#c18c78] rounded-full mt-2"></div>
            </div>
 ...
 className="text-2xl font-bold text-[#c18c78]"
                  />
                </div>
              ) : (
                <PriceDisplay 
 ...
 <div className={`text-gray-600 ${productOptionsPriceModifier > 0 ? 'text-[#c18c78]' : 'text-red-600'}`}>
                          {t('product_options')}: {productOptionsPriceModifier > 0 ? '+' : ''}{formatPrice(productOptionsPriceModifier)}
                        </div>
                      )}
                      {selectedAddOns.length > 0 && (
 <div className="text-[#c18c78]">{t('addons')}: +{formatPrice(getAddOnsPrice())}</div>
                      )}
                    </div>
 <div className="text-xl font-bold text-[#c18c78]">
 ...
 <Sparkles className="w-5 h-5 text-[#c18c78]" />
                  {t('product_options')}
                </h3>
            ...
            {product.addOns && product.addOns.length > 0 && (
              <div className="pt-4">
                <h3 className="text-lg font-bold text-[#0A2A55] mb-3 flex items-center gap-2">
 <Gift className="w-5 h-5 text-[#c18c78]" />
                  {t('addons')}
                </h3>
 ...
 isSelected ? 'border-[#c18c78] bg-[#d9a890]/5' : 'border-gray-200 hover:border-[#c18c78]'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 flex-1">
                            <div className={`w-5 h-5 rounded border flex items-center justify-center ${
 isSelected ? 'border-[#c18c78] bg-[#d9a890]' : 'border-gray-400'
                            }`}>
 ...
 <div className="text-lg font-bold text-[#c18c78]">+{formatPrice(addOn.price)}</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="pt-4">
              <h3 className="text-lg font-bold text-[#0A2A55] mb-3 flex items-center gap-2">
 <span className="w-5 h-5 bg-[#d9a890]/20 rounded flex items-center justify-center text-[#c18c78] text-sm">📎</span>
              ...
              <div className="space-y-4">
                <textarea
                  ...
                  rows={2}
 className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#c18c78] focus:border-[#c18c78]"
                  placeholder={t('notes_placeholder')}
                />
                <div className="flex flex-wrap gap-2">
 <label className="inline-block px-4 py-2 bg-white border border-dashed border-gray-400 rounded-md cursor-pointer hover:border-[#c18c78]">
 <span className="text-[#c18c78]">📷</span> {t('add_images')}
                    <input
                      type="file"
 ...
 className="flex-1 flex items-center justify-center gap-2 bg-[#d9a890] text-white px-6 py-3 rounded-md font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {addingToCart ? (
                    <>
 ...
 className="inline-block px-5 py-2 bg-[#d9a890] text-white rounded-md hover:bg-[#c18c78]"
                >
                  {t('login')}
                </button>
              </div>
 ...
 <div className="w-10 h-10 bg-[#d9a890]/10 rounded-full flex items-center justify-center">
 <User className="w-5 h-5 text-[#c18c78]" />
                  </div>
                  <div>
 ...
 className="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#c18c78] focus:border-[#c18c78]"
                    rows={4}
                    maxLength={500}
                  />
 ...
 className="inline-flex items-center gap-2 px-5 py-2 bg-[#d9a890] text-white rounded-md disabled:opacity-50"
                >
                  {isSubmittingComment ? (
                    <>
                      <div className="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
          ...
          <div className="space-y-4">
            {commentsLoading ? (
              <div className="text-center py-6">
 <div className="inline-block w-6 h-6 border-2 border-[#c18c78] border-t-transparent rounded-full animate-spin"></div>
